...
type TableBodyProps = TableHeaderProps & TableBodyContent;
type Props = TableHeaderProps & TableBodyProps;
    ...
    <>
      <table className="hidden w-full table-fixed rounded-lg text-center text-base shadow sm:table [&_tr]:h-12">
        ...
        <TableBody
          Header={Header}
          Content={Content}
          speacialRow={speacialRow}
        />
      </table>
      <SmallDevTable Content={Content} speacialRow={speacialRow} />
    </>
  );
};
      ...
      return (
        <tr key={index} className="even:bg-gray-50 hover:bg-gray-100">
          {rowDatas.texts.map((text: string, text_idx: number) => {
            return (
              <td key={text + index + text_idx} className="whitespace-pre-line">
                {text}
              </td>
            );
          })}
...
const SmallDevTable: React.FC<TableBodyContent> = ({
  Content,
  speacialRow,
}: TableBodyContent) => (
            ...
            <div className="divide-y-2">
              {texts.map((text: string, text_idx: number) => {
                return text_idx === 0 ? (
                  ...
                  <p key={text + text_idx} className="whitespace-pre-wrap py-1">
                    {text}
                  </p>
                );
              })}
